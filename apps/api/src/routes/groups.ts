...
groupsRouter.openapi(
  createRoute({
    ...
    responses: {
      200: {
        content: { 'application/json': { schema: z.array(GroupSummarySchema) } },
  ...
  async (c) => {
    ...
    const rows = await db
      .select({
        id: watchGroups.id,
        name: watchGroups.name,
        createdAt: watchGroups.createdAt,
...
groupsRouter.openapi(
  createRoute({
    ...
    responses: {
      ...
      400: {
        content: { 'application/json': { schema: ErrorSchema } },
        description: 'Cannot remove the last host',
      },
      401: {
        content: { 'application/json': { schema: ErrorSchema } },
        description: 'Not authenticated',
...
groupsRouter.openapi(
  createRoute({
    ...
    responses: {
      200: {
        content: { 'application/json': { schema: z.array(WatchPartySchema) } },
